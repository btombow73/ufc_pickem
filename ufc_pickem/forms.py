...
class LoginForm(FlaskForm):
    email = StringField('Email or Username', validators=[DataRequired()])  # can use email or username to log in
    password = PasswordField('Password', validators=[DataRequired()])
...
class FightForm(FlaskForm):
    ...
    favorite = SelectField('Favorite Fighter', choices=[('fighter1', 'Fighter 1'), ('fighter2', 'Fighter 2')], validators=[DataRequired()])
...
class FightResultForm(FlaskForm):
    winner = SelectField('Winner', choices=[], validators=[DataRequired()])  # set dynamically based on fighters
